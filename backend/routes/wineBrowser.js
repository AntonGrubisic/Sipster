...
const {getAllWines, filterByQuery} = require('../services/wineBrowserService');
...
router.get('/health', async (_req, res) => {
    try {
        ...
        res.json({
            ok: true, items: count, note: 'Data served from in-memory cache with stale-while-revalidate.'
        });
    } catch (err) {
...
router.get('/search', async (req, res) => {
    ...
    try {
        ...
        if (filtered.length === 0) {
            return res.json({
                query: q, count: 0, total: 0, limit, offset, results: [], message: `No wines matched "${q}".`
            });
        }
